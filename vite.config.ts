...
const RSS_APNEWS_FALLBACK_ENABLED = process.env.RSS_APNEWS_FALLBACK === '1';
const RSS_APNEWS_FALLBACK_URL = (process.env.RSS_APNEWS_FALLBACK_URL || 'https://rsshub.app/apnews').trim();

function splitDomainList(raw?: string): string[] {
  if (!raw) return [];
  return raw
    .split(/[,\s]+/)
...
function safeHostname(urlLike: string): string | null {
  try {
...
if (RSS_APNEWS_FALLBACK_ENABLED) {
  const fallbackHost = safeHostname(RSS_APNEWS_FALLBACK_URL);
  if (fallbackHost) RSS_PROXY_ALLOWED_DOMAINS.add(fallbackHost);
}
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          const isApNewsGoogleFeed = parsed.hostname === 'news.google.com' && /apnews\.com/i.test(feedUrl);
          const tryApNewsFallback = async () => {
            if (!RSS_APNEWS_FALLBACK_ENABLED || !isApNewsGoogleFeed) return null;
            const fallbackResp = await fetch(RSS_APNEWS_FALLBACK_URL, {
              headers: {
            ...
            const fallbackType = fallbackResp.headers.get('content-type') || 'application/xml';
            return { fallbackResp, fallbackBody, fallbackType };
          };
          ...
          let bodyToSend = await responseToSend.text();
          let contentType = responseToSend.headers.get('content-type') || 'application/xml';
          let usedFallback = false;

          if (isApNewsGoogleFeed && RSS_APNEWS_FALLBACK_ENABLED) {
            const looksBlockedHtml =
              /text\/html/i.test(contentType) ||
              /^\s*</.test(bodyToSend) && /<html|<!doctype html/i.test(bodyToSend.slice(0, 240));
            if (responseToSend.status >= 400 || looksBlockedHtml) {
              try {
                const fallback = await tryApNewsFallback();
                if (fallback?.fallbackResp.ok) {
                  responseToSend = fallback.fallbackResp;
                  bodyToSend = fallback.fallbackBody;
                  contentType = fallback.fallbackType;
                  usedFallback = true;
                }
              } catch {
                // Keep primary response if fallback fails.
              }
            }
          }
          ...
          res.setHeader('X-RSS-Proxy-Source', usedFallback ? 'apnews-fallback' : 'primary');
          res.end(bodyToSend);
        } catch (error: any) {
          ...
          try {
            const parsed = new URL(feedUrl);
            const isApNewsGoogleFeed = parsed.hostname === 'news.google.com' && /apnews\.com/i.test(feedUrl);
            if (RSS_APNEWS_FALLBACK_ENABLED && isApNewsGoogleFeed) {
              const fallbackResp = await fetch(RSS_APNEWS_FALLBACK_URL, {
                headers: {
                  'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
                  'Accept': 'application/rss+xml, application/xml, text/xml, */*',
              ...
              if (fallbackResp.ok) {
                const fallbackBody = await fallbackResp.text();
                const fallbackType = fallbackResp.headers.get('content-type') || 'application/xml';
                res.statusCode = fallbackResp.status;
                res.setHeader('Content-Type', fallbackType);
                res.setHeader('Cache-Control', 'public, max-age=300');
                res.setHeader('Access-Control-Allow-Origin', '*');
                res.setHeader('X-RSS-Proxy-Source', 'apnews-fallback');
